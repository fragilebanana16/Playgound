...
1、确保每个mesh都包含同名uv map，Ctrl J合并

2、全选uv展开，烘焙时确保物体和新增的空白纹理选中，不要预览新建的uv map但是要选择

3、metal较特殊，将原来的金属度连接到emmissn烘焙后，得到的纹理连接metal(不连emission)

除非你完全跟着教程走，否则会出现奇怪的问题

1、我只想烘焙一个，不行需要每个材质都放，否则是黑的



手头有个下载的模型想用PBR在OpenGL显示，但是贴图太多，OpenGL也没法实现Blender里的节点系统解析，只能输出核心的几个贴图，但是吧，网络上的模型一个顶点太多UV动辄几百万，uv展都展不开，如果只写代码，这一块始终没法打通，总要有个美术要配合你，这好吗这不好。

拿到模型先观察，我直接Ctrl+J合并，注意uvmap要同名，然后新建个纹理展开，有条件用插件展一下，有重叠，但我觉得问题不大，谁让我不是美术呢

然后咱们来烘焙漫反射，因为材质太多，我们去掉一些影响比例小的材质，节省工作量

新建贴图，保持白色选中状态，注意这里必须全部材质都得加，否则会出现黑色，另外这里由于一些不可抗力，metal要先全部归零，按着做就完了，哪有那么多为什么

之后就是重复性工作了

之后我们烘焙metal，这里需要曲线救国，用emission烘焙再接到metal，别问，问就是我也不知道，我不是美术

之后normal和roughness也是类似的，这里坑就少了



2 上节说到不会美术很难打通这套操作流，这下我们用现成的OpenGL加载一下

卧槽，怎么是画的，难道说、嗯？难道说？好吧
...
基于图像的光照(IBL)是一类光照技术的集合

> 在 Blender 里看到的 **“采样数 (samples)”**，其实就是指 **在微平面半球上取==多少个方向==来做积分近似
>

#### PBR

粗糙度决定：有多少微表面法线朝向半程向量 h (光线向量l和视线向量v夹角)，粗糙度越大，微平面分布越宽，因此朝向 h 的微平面数量“相对更多”

#####  入射辐射率 Li(p,ωi)

- **定义：**
$$
Li(p,ωi)=d2ΦdA⋅cos⁡θ dω
$$
表示在点 p 上，沿着入射方向 ωi，单位面积（投影到垂直于 ωi 的面）、单位立体角上的辐射通量密度。

- **物理意义：**
  - 它度量了“从某个方向射入点 p 的光有多强”。
  - 单位是 W/(m2⋅sr)。
  - 在渲染方程中，所有入射光的贡献都通过 Li 来表达。

##### 与其他量的关系

- **入射辐照度微元：**
$$
dEi(p,ωi)=Li(p,ωi) (n⋅ωi) dωi
$$
这是把入射辐射率转化为表面点的辐照度贡献。

- **总辐照度：**
$$
E(p)=∫ΩLi(p,ωi) (n⋅ωi) dωi
$$

- BRDF 定义：
$$
fr(p,ωi,ωo)=dLo(p,ωo)/dEi(p,ωi)
$$
它表达了：某个入射方向的辐照度微元 dEi，如何被材料映射为某个出射方向的辐射率增量 dLo。
所以 fr 就是“入射 → 出射”的方向性映射关系。

把两者结合：
$$
dLo(p,ωo)=fr(p,ωi,ωo) Li(p,ωi) (n⋅ωi) dωi
$$
再对半球所有入射方向积分：
$$
...
PBR 的目标是用物理正确的方式计算出物体表面的出射辐射亮度 Lo。核心公式是 **渲染方程**：
$$
Lo(p,ωo)=∫Ωfr(ωi,ωo) Li(p,ωi) (n⋅ωi) dωi
$$
其中：
...
Lo是某个点 p 在观察方向 ωo 上的 **物理亮度**,它本身是一个标量。

> **BRDF (Bidirectional Reflectance Distribution Function)**：只负责描述材质的散射行为（漫反射 + 镜面反射）。它告诉我们：光从某个入射方向 ωi 到某个出射方向 ωo 的比例是多少。
假设某点的出射亮度 Lo=1.0。材质颜色 ρ=(0.8,0.2,0.1)。那么RGB 就是：
$$
(R,G,B)linear=(0.8,0.2,0.1)⋅1.0=(0.8,0.2,0.1)
$$
###### 漫反射纹理 vs Albedo 图：核心区别

1. 传统 Blinn‑Phong / Lambert 时代：漫反射纹理（Diffuse Map）
在早期的非物理渲染模型里：Diffuse Map = 漫反射颜色，它包含：物体的固有颜色、环境光 baked 信息、AO、阴影、光照、甚至高光残留， 也就是说，它经常是“脏的”，带有光照信息。

2. PBR 时代：Albedo（或 BaseColor）
在 PBR（尤其是金属度/粗糙度 workflow）中：Albedo = 纯粹的表面反照率（Reflectance）， 它必须是“干净的”：不包含阴影、AO 、高光、环境光 、 baked 光照
...
对于铺屏四边形(Screen-filled Quad)上的每一个片段，我们都会根据周边深度值计算一个**遮蔽因子(Occlusion Factor)**。这个遮蔽因子之后会被用来减少或者抵消片段的环境光照分量。遮蔽因子是通过采集片段周围球型核心(Kernel)的多个深度样本，并和当前片段深度值对比而得到的。高于片段深度值样本的个数就是我们想要的遮蔽因子。
...
###### 浮点帧缓冲
OpenGL 的 HDR 是：在浮点帧缓冲中进行高亮度光照计算，然后通过 Tone Mapping 压缩到 0–1 输出。

浮点帧缓冲可以存储超过0.0到1.0范围的浮点值，

```c++
// GL_RGB16F
glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB16F, SCR_WIDTH, SCR_HEIGHT, 0, GL_RGB, GL_FLOAT, NULL); 
```
帧缓冲默认一个颜色分量只占用8位（GL_RGB）

#### 法线贴图

纹理中的颜色向量用r、g、b元素代表一个3D向量。类似的我们也可以将法线向量的x、y、z元素储存到纹理中，代替颜色的r、g、b元素。
整体偏蓝是因为法线的指向都偏向z轴（0, 0, 1）这是一种偏蓝的颜色（RGB的B），偏绿指向正y轴方向（0, 1, 0），红色类似。

#### 不先写use，uniform 能设置进去吗

不能，OpenGL 仍然会把 uniform 设置到“当前绑定的 program”

#### 阴影映射

1.渲染深度贴图（从光源的视角来渲染场景，并把生成的深度值储存到纹理）
2.渲染场景
3.使用生成的深度贴图来计算片段是否在阴影之中
