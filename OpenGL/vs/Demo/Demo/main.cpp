...
int main()
{
    ...
    Shader debugFbo("8.1.fbo_debug.vs", "8.1.fbo_debug.fs");
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        {
            glm::mat4 tempModel = glm::translate(model, glm::vec3(0, 0, 5.0 * i));
            tempModel = glm::rotate(tempModel, glm::radians(10.0f * modelX), glm::vec3(0, 1, 0));
            shaderGeometryPass.setMat4("model", tempModel);
            ourModel.Draw(shaderGeometryPass);

            // Y 轴镜像
            glm::mat4 mirrorY = glm::scale(glm::mat4(1.0f), glm::vec3(-1.0f, 1.0f, 1.0f));
            shaderGeometryPass.setMat4("model", mirrorY * tempModel);
            ourModel.Draw(shaderGeometryPass);
        }
        glBindFramebuffer(GL_FRAMEBUFFER, 0);
        ...
        // debug 
        debugFbo.use();
        debugFbo.setInt("gPosition", 0);
        debugFbo.setInt("gNormal", 1);
        debugFbo.setInt("gAlbedoSpec", 2);
        ...
        {
            debugFbo.setVec3("lights[" + std::to_string(i) + "].Position", lightPositions[i] + lightPos);
            debugFbo.setVec3("lights[" + std::to_string(i) + "].Color", lightColors[i]);
            // update attenuation parameters and calculate radius
            const float linear = 0.7f;
            const float quadratic = 1.8f;
            debugFbo.setFloat("lights[" + std::to_string(i) + "].Linear", linear);
            debugFbo.setFloat("lights[" + std::to_string(i) + "].Quadratic", quadratic);
            debugFbo.setFloat("lights[" + std::to_string(i) + "].LightIntensity", lightIntensity);
        }
        debugFbo.setVec3("viewPos", camera.Position);
        // finally render quad
        renderQuad();
        ...
        {
            model = glm::mat4(1.0f);
            model = glm::translate(model, lightPositions[i] + lightPos);
