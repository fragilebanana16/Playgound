...
namespace GuitarInXml.Controls
{
    ...
    public class SingleString : UserControl
    {
        /// <summary>
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            g.DrawLine(
                new Pen(new SolidBrush(lineColor)), 
                new PointF(1, Padding.Top + (this.Height - Padding.Top - Padding.Bottom) / 2), 
                new PointF(this.Width - 1, Padding.Top + (this.Height - Padding.Top - Padding.Bottom) / 2));

            int numberInterval = 10; // 音间隔距离
            int sectorInterval = 30; // 小节间隔距离
            int sectorCount = 0; // 已有几个小节了

            int currentPos = this.Padding.Left;
            using (SolidBrush blueBrush = new SolidBrush(Color.Red))
            {
                for (int i = 1; i < 20; i++)
                {
                    e.Graphics.FillRectangle(
                        blueBrush,
                        new Rectangle(currentPos, Padding.Top, this.Height - this.Padding.Vertical, this.Height - this.Padding.Vertical));

                    // 遇到小节,(this.Height - this.Padding.Vertical + numberInterval)为音区域的大小，正方形
                    if (0 == i % 4)
                    {
                        currentPos += (this.Height - this.Padding.Vertical + numberInterval) + sectorInterval;
                        sectorCount++;
                    }
                    else
                    {
                        currentPos += (this.Height - this.Padding.Vertical + numberInterval);
                    }
                    // TODO：路径需要判断双击编辑音，最大位数两位，到30
                }
            }
            ...
            //g.DrawLine(
            //    new Pen(new SolidBrush(lineColor)), 
            //    new PointF(Padding.Left + size.Width + 1, Padding.Top + (this.Height - Padding.Top - Padding.Bottom) / 2), 
            //    new PointF(Width - Padding.Right, Padding.Top + (this.Height - Padding.Top - Padding.Bottom) / 2));
        }
    }
}
