...
// 预加载封面
const displayCover = ref(currentSong.value.cover || altCover)
watch(() => currentSong.value.cover, (newCover) => {
  const img = new Image()
  img.onload = () => displayCover.value = newCover
  img.onerror = () => displayCover.value = altCover
  img.src = newCover || altCover
}, { immediate: true })
...
<template>
  <footer class="footer-player border-1 border-x-0 border-b-0 shadow-sm border-t-blue-100">
    <div class="mx-auto flex items-center justify-between px-4 py-2">
      <div class="flex items-center gap-4">
        <div class="flex items-center gap-2 w-[240px]">
          <img
            :src="displayCover"
            class="rounded-md w-9 h-9 object-cover"
