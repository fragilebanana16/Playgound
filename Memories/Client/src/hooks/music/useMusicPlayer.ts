...
export function useMusicPlayer() {
  ...
  const handleAudioError = async () => {
    ...
    try {
      ...
      audio.src ='/test.mp3'//  rows[0].url;
      // audioStore.setCurrentSongUrl('/test.mp3')
      audio.load()
      play()
    } catch (e) {
  ...
  function setPlaylist(songs) {
    musicStore.addTrackAndPlay(songs)
  }
  ...
  const playSong = async (song) => {
    const existingIndex = musicStore.trackList.findIndex(
      (existingTrack) => existingTrack.id === song.id
    )
    if (existingIndex === -1) {
      musicStore.addTrackAndPlay(song)
    } else  {
  ...
  function playNext() {
    switch (playMode.value) {
      ...
      default: // 对于顺序播放和列表循环模式，播放列表中的下一首歌
        ...
        Loadlyrics()
        play();
        const song = musicStore.trackList[nextIndex]
        if (song) {
            musicStore.addToHistory(song)
        }
        break;
  ...
  function playRandomSong() {
    ...
    audio.src = currentSong.value.source; // 更新audio元素的资源地址
    play();
    const song = musicStore.trackList[randomIndex]
    if (song) {
        musicStore.addToHistory(song)
    }
  }
